...
class EasyDataCleaningGrader(Grader):
    """Task-specific grader for easy basic data-cleaning task."""
    
    def grade(self) -> GradeResult:
        """Easy task grading with strict criteria for nulls and duplicates."""
        if self._task_config is None:
            raise RuntimeError("Grader not set up. Call setup() first.")
        ...
        # Easy task: Strict scoring for basic requirements
        for criterion, weight in criteria.items():
            score = self._evaluate_criterion(criterion)
            # Penalty for not handling basic requirements properly
            if criterion in ["null_handling", "duplicate_handling"] and score < 0.9:
                score *= 0.7
            breakdown[criterion] = round(score, 4)
            total_score += score * weight
            total_weight += weight
...
class MediumDataCleaningGrader(Grader):
    """Task-specific grader for medium intermediate data-cleaning task."""
    
    def grade(self) -> GradeResult:
        """Medium task grading with balanced criteria and email validation."""
        if self._task_config is None:
            raise RuntimeError("Grader not set up. Call setup() first.")
        ...
        # Medium task: Partial credit for progress
        for criterion, weight in criteria.items():
            score = self._evaluate_criterion(criterion)
            # Bonus for good email validation
            if criterion == "email_validation" and score > 0.8:
...
class HardDataCleaningGrader(Grader):
    """Task-specific grader for hard advanced data-cleaning task."""
    
    def grade(self) -> GradeResult:
        """Hard task grading with comprehensive pipeline validation."""
        if self._task_config is None:
            raise RuntimeError("Grader not set up. Call setup() first.")
        ...
        # Hard task: Comprehensive evaluation with all criteria
        for criterion, weight in criteria.items():
            score = self._evaluate_criterion(criterion)
            # Reward complete pipelines
            if criterion in ["type_conversion", "normalization"] and score > 0.7:
